...
2) Need a suitable plot grid to be in the foreground. Perhaps a thin green line may work with the viridis::plasma pallet.
...
the.breaks<-c(0,6,12,18,24,30,36,43) 
  the.labels<-c("0 - 6","6 - 12","12 - 18", "18 - 24", "24 - 30","30 - 36", "36 - 42") #breaks are (included,excluded] 
  ...
  df_byPeriod %>% 
   ...
   scale_fill_manual(values=color.pal,name="NIS with Suitable Habitat",labels=factor(the.labels)) +
    ...
    scale_y_continuous(breaks=seq(from=51,to=67,by=2),name="Latitude (°N)",expand=c(0,0))+
...
Plot change in
```{r change}
df_byPeriod0 <- df_byPeriod %>% 
  filter(Period == 0) %>% 
  arrange(Week,lat)

df_byPeriod1 <- df_byPeriod %>% 
  filter(Period == 1) %>% 
  arrange(Week,lat)

df_byPeriod1$speciesRichness <- df_byPeriod1$speciesRichness-df_byPeriod0$speciesRichness
df_byPeriod1$Period = 2

color.pal <- brewer.pal(11,"RdYlBu")[c(9,7,4,3,2,1)]

the.breaks<-c(-10,-0.4,0.67,8) 
  the.labels<-c("Suitable for Less NIS","No Change","Suitable for More NIS") #breaks are (included,excluded] 
