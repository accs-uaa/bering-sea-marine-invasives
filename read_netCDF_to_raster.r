## D:\HelpingOthers\ABSIlcc\NFWF2016\Models\rCode\read_netCDF_to_raster.r
...
for (f in ff){  ## Process one at a time...
#f <- ff[1] ##for debugging
	...
	for(Var in vars){ ## handle both variables.
		...
		sgdf.Max <- foreach(i = 1:nrow(TimeSteps), .combine = 'cbind', .packages = 'raster') %dopar% { 
				...
				bMax <- max(b, na.rm=T)  	## summarize each point in the raster, taking the maximum temperature and casting result as a raster.
